...
mmc_bwrite_mass_pro(int dev_num, ulong start, lbaint_t blkcnt, const void*src)
{
	...
	if ( (mmc->boot_support) && (start == BOOT0_SDMMC_START_ADDR) )//(mmc->part_support & PART_SUPPORT)
	{
		if (mmc_switch_part(dev_num, MMC_SWITCH_PART_BOOT_PART_1)) {
			printf("switch to boot1 partition failed\n");
			return 0;
		}
	...
	if ((mmc->boot_support)  && (start == BOOT0_SDMMC_START_ADDR) )//(mmc->part_support & PART_SUPPORT)
	{
		...
		if (mmc_switch_part(dev_num,  0)) {
			printf("switch to user partition failed\n");
			return 0;
		}
...
int mmc_send_op_cond(struct mmc *mmc)
{
	...
	if (timeout < 0){
		printf("wait for mmc init failed\n");
		return UNUSABLE_ERR;
	}
...
int mmc_send_ext_csd(struct mmc *mmc, char *ext_csd)
{
	...
	if(err)
		printf("mmc send ext csd failed\n");
...
int mmc_change_freq(struct mmc *mmc)
{
	...
	if (err){
		printf("mmc change to hs failed\n");
		return err;
	}
...
int mmc_switch_boot_bus_cond(int dev_num, u32 boot_mode, u32 rst_bus_cond, u32 bus_width)
{
	...
	if(ret){
		printf("switch bus cond failed\n");
		return -1;
	}
	ret = mmc_send_ext_csd(mmc, ext_csd);
	if(ret){
		printf("send ext csd failed\n");
		return -1;
	}
	printf("boot bus cond:0x%x\n",ext_csd[EXT_CSD_BOOT_BUS_COND]);
	if(boot_bus_cond!=ext_csd[EXT_CSD_BOOT_BUS_COND])	{
		printf("Set boot bus cond failed,now bus con is 0x%x\n",ext_csd[EXT_CSD_BOOT_BUS_COND]);
		return -1;
	}
...
int mmc_switch_boot_part(int dev_num, u32 boot_ack, u32 boot_part)
{
	...
	if(ret){
		printf("switch boot partd failed\n");
		return -1;
	}
	ret = mmc_send_ext_csd(mmc, ext_csd);
	if(ret){
	...
	printf("part conf:0x%x\n",ext_csd[EXT_CSD_PART_CONF]);
	if(part_config!=ext_csd[EXT_CSD_PART_CONF])	{
		printf("switch boot part failed,now part conf is 0x%x\n",ext_csd[EXT_CSD_PART_CONF]);
		return -1;
	}
...
int mmc_switch_part(int dev_num, unsigned int part_num)
{
	...
	printf("part conf:0x%x\n",ext_csd[EXT_CSD_PART_CONF]);
	if(part_config!=ext_csd[EXT_CSD_PART_CONF])	{
		printf("switch boot part failed,now bus con is 0x%x\n",ext_csd[EXT_CSD_PART_CONF]);
		return -1;
	}
	mmc->part_config = part_config;

	printf("switch part succeed\n");
	return ret;
...
retry_scr:
	...
	while (timeout--) {
		...
		if (err){
			printf("Check high speed status faild\n");
			return err;
		}
	...
	if (err){
		printf("switch to high speed failed\n");
		return err;
	}
...
int mmc_startup(struct mmc *mmc)
{
	...
	if (err){
		printf("Put the Card in Identify Mode failed\n");
		return err;
	}
	...
	if (!IS_SD(mmc) && (mmc->version >= MMC_VERSION_4)) {
		...
		if (ext_csd[226])//ext_csd[160] & PART_SUPPORT
		{
		  mmc->boot_support = 1;//lwj
			...
			printf("PART_SUPPORT mmc->boot1_lba = %d\n",mmc->boot1_lba);
		}
		else
		{
			printf("not PART_SUPPORT ext_csd[226] = %d\n",ext_csd[226]);
		}
	}
	...
	if (err){
		printf("Change speed mode failed\n");
		return err;
	}
	...
	if (IS_SD(mmc)) {
		if (mmc->card_caps & MMC_MODE_4BIT) {
			...
			err = mmc_send_cmd(mmc, &cmd, NULL);
			if (err){
				printf("sd set bus width failed\n");
				return err;
			}
		...
		if (mmc->card_caps & MMC_MODE_4BIT) {
			...
			if (err){
				printf("mmc switch bus width failed\n");
				return err;
			}
			...
			if (err){
				printf("mmc switch bus width8 failed\n");
				return err;
			}
...
/*    printf("---------------ext_csd[192] = %d--------------\n", ext_csd[192]);*/
	...
	{
		case 0:
			printf("---------------MMC version 4.0---------------\n");
			break;
		case 1:
			printf("---------------MMC version 4.1---------------\n");
			break;
		case 2:
			printf("---------------MMC version 4.2---------------\n");
			break;
		case 3:
			printf("---------------MMC version 4.3---------------\n");
			break;
		case 4:
			printf("---------------MMC version Obsolete----------\n");
			break;
		case 5:
			printf("---------------MMC version 4.41--------------\n");
			break;
		case 6:
			printf("---------------MMC version 4.5---------------\n");
			break;
		case 7:
			printf("---------------MMC version 5.0---------------\n");
			break;	
		default:
			printf("---------------MMC version not define--------\n");
			break;
	}
	printf("---------------mmc->clock %d-----------\n", mmc->clock);
	printf("---------------mmc->bus_width %d--------------\n", mmc->bus_width);
	printf("boot0 capacity: %dKB,boot1 capacity: %dKB\n"
...
int mmc_send_if_cond(struct mmc *mmc)
{
	...
	if (err){
		printf("mmc send if cond failed\n");
		return err;
	}
